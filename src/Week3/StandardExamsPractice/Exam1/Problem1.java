package Week3.StandardExamsPractice.Exam1;
...
public class Problem1 {
    ...
    public static List<String> elementsInJustOne(List<String> list1, List<String> list2) {
        ...
        return Stream.concat(
                        list1.stream().filter(i -> !list2.contains(i)),
                        list2.stream().filter(i -> !list1.contains(i))
                )
                .toList();
    }
    ...
    public static List<String> getAllFairfieldCustomers(List<Customer> list) {
        //implement
        return list.stream().filter(c -> c.getCity().equals("Fairfield")).map(Customer::getName).toList();
    }
